  ...
  return (
    <div className="page-shell">
      <section className="surface px-6 py-8 sm:px-8">
        ...
        <h1 className="font-display text-4xl font-bold text-[var(--text)] sm:text-5xl">Open to collaborate on React, APIs, AI integrations, and modern product builds.</h1>
      ...
      <section className="card-grid cols-2">
        ...
        {profile?.email ? (
          <a href={`mailto:${profile.email}`} className="surface px-6 py-7 transition hover:bg-[#f7f9fb]">
            <p className="text-sm uppercase tracking-[0.2em] text-[var(--accent-secondary)]">Email</p>
            <p className="mt-3 font-display text-2xl font-bold text-[var(--text)]">{profile.email}</p>
            <p className="mt-3 text-[var(--muted)]">Best for project inquiries, collaboration, and hiring conversations.</p>
          </a>
        ) : null}
        {profile?.phone ? (
          <a href={`tel:${profile.phone.replace(/\s+/g, "")}`} className="surface px-6 py-7 transition hover:bg-[#f7f9fb]">
            <p className="text-sm uppercase tracking-[0.2em] text-[var(--accent-secondary)]">Phone</p>
            <p className="mt-3 font-display text-2xl font-bold text-[var(--text)]">{profile.phone}</p>
            <p className="mt-3 text-[var(--muted)]">Available for direct conversations around active project work.</p>
          </a>
        ) : null}
        {profile?.github ? (
          <a href={profile.github} target="_blank" rel="noreferrer" className="surface px-6 py-7 transition hover:bg-[#f7f9fb]">
            <p className="text-sm uppercase tracking-[0.2em] text-[var(--accent-secondary)]">GitHub</p>
            <p className="mt-3 font-display text-2xl font-bold text-[var(--text)]">Source Profile</p>
            <p className="mt-3 text-[var(--muted)]">Code, experiments, and implementation history.</p>
          </a>
        ) : null}
        {profile?.linkedin ? (
          <a href={profile.linkedin} target="_blank" rel="noreferrer" className="surface px-6 py-7 transition hover:bg-[#f7f9fb]">
            <p className="text-sm uppercase tracking-[0.2em] text-[var(--accent-secondary)]">LinkedIn</p>
            <p className="mt-3 font-display text-2xl font-bold text-[var(--text)]">Professional Profile</p>
