  ...
  async function sendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/community", {
        ...
        body: JSON.stringify({ body: form.get("message"), authorName: form.get("name"), parentId: replyTo }),
      });
      const data = await response.json();
      if (!response.ok) {
    ...
    <section className="community-wall surface px-6 py-7 sm:px-8">
      ...
      <form className="community-form" onSubmit={sendMessage}>
        <div className="reply-status">
          {replyTo ? (
            <span>
              ...
              <button type="button" disabled={sending} onClick={() => setReplyTo(null)}>
                Cancel
              </button>
            </span>
          ) : (
            <span>{accessToken ? "Posting as a signed-in guest" : "Sign in to join the conversation"}</span>
          )}
        </div>
        ...
        <textarea
          ...
          maxLength={1000}
          rows={3}
          placeholder={accessToken ? "Write a message…" : "Sign in to write a message"}
        />
          ...
          {roots.map((message) => (
            <article className="community-message" key={message.id}>
              ...
              <div className="message-main">
                ...
                <p>{message.body}</p>
                <button className="reply-button" onClick={() => setReplyTo(message.id)}>
                  Reply
                </button>
